...
public abstract class UIScreen
{
    ...
    /// <summary>Handle a printable character pressed while this screen is active.</summary>
    public virtual bool HandleChar(char c) => false;

    /// <summary>Find a text field anywhere in the widget tree by <see cref="Widget.Name"/>.</summary>
    public TextField? FindTextField(string name)
    {
        foreach (Widget root in _roots)
        {
            TextField? match = FindTextFieldInWidget(root, name);
            if (match != null)
                return match;
        }
    ...
    private static TextField? FindTextFieldInWidget(Widget widget, string name)
    {
        if (widget is TextField field && field.Name == name)
            return field;

        for (int i = widget.Children.Count - 1; i >= 0; i--)
        {
            TextField? childMatch = FindTextFieldInWidget(widget.Children[i], name);
            if (childMatch != null)
                return childMatch;
        }
    ...
    public IUIButton? FindHoveredButton()
    {
        if (!Visible) return null;

        IUIButton? hovered = null;
        for (int i = _roots.Count - 1; i >= 0; i--)
    ...
    private static IUIButton? FindHoveredButtonInWidget(Widget widget)
    {
        if (widget is IUIButton { IsHovered: true } button)
            return button;

        for (int i = widget.Children.Count - 1; i >= 0; i--)
        {
            IUIButton? childMatch = FindHoveredButtonInWidget(widget.Children[i]);
            if (childMatch != null)
                return childMatch;
        }
    ...
    public IUIButton? FindButton(string name)
    {
        foreach (Widget root in _roots)
        {
            IUIButton? match = FindButtonInWidget(root, name);
            if (match != null)
                return match;
        }
    ...
    private static IUIButton? FindButtonInWidget(Widget widget, string name)
    {
        if (widget is IUIButton button && widget.Name == name)
            return button;

        for (int i = widget.Children.Count - 1; i >= 0; i--)
        {
            IUIButton? childMatch = FindButtonInWidget(widget.Children[i], name);
            if (childMatch != null)
                return childMatch;
        }
